...
class GreatWall_Pro_Employees extends WP_Widget {
	...
	public function __construct() {
		$widget_ops = array('classname' => 'greawall_pro_employees_widget', 'description' => __( 'Display your team members in a stylish way.', 'greatwall') );
        parent::__construct(false, $name = __('Home - Employees', 'greatwall'), $widget_ops);
	...
	<p><?php _e('In order to display this widget, you must first add some employees from the dashboard. Add as many as you want and the theme will automatically display them all.', 'greatwall'); ?></p>
	<p>
	<label for="<?php echo $this->get_field_id('title'); ?>"><?php _e('Title', 'greatwall'); ?></label>
	<input class="widefat" id="<?php echo $this->get_field_id('title'); ?>" name="<?php echo $this->get_field_name('title'); ?>" type="text" value="<?php echo $title; ?>" />
	</p>
	<p><label for="<?php echo $this->get_field_id( 'number' ); ?>"><?php _e( 'Number of employees to show (-1 shows all of them):', 'greatwall' ); ?></label>
	<input id="<?php echo $this->get_field_id( 'number' ); ?>" name="<?php echo $this->get_field_name( 'number' ); ?>" type="text" value="<?php echo $number; ?>" size="3" /></p>
    <p><label for="<?php echo $this->get_field_id('see_all'); ?>"><?php _e('Enter an URL here if you want to section to link somewhere.', 'greatwall'); ?></label>
    ...
    <p><label for="<?php echo $this->get_field_id('see_all_text'); ?>"><?php _e('The text for the button [Defaults to <em>See all our employees</em> if left empty]', 'greatwall'); ?></label>
	...
	<p><label for="<?php echo $this->get_field_id( 'category' ); ?>"><?php _e( 'Enter the slug for your category or leave empty to show all employees.', 'greatwall' ); ?></label>
...
?>
		...
		<?php if ($see_all != '') : ?>
			<a href="<?php echo esc_url($see_all); ?>" class="roll-button more-button">
				...
				<?php else : ?>
					<?php echo __('See all our employees', 'greatwall'); ?>
				<?php endif; ?>
			</a>
		<?php endif; ?>	
