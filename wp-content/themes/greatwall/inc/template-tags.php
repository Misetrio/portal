...
function greatwall_pro_get_post_views($postID){
    ...
    return '<span class="view-count">' . number_format($count) . '</span> ' . __('Views', 'greatwall');
}
...
function greatwall_pro_custom_meta_box_markup($object)
{
    ...
    ?>
        <div>
            <label for="is_featured"><?php echo __('Featured this post on blog slider', 'greatwall'); ?> </label>
            <?php
...
function greatwall_pro_first_category() {
    ...
    if ($category) {
      echo '<a href="' . get_category_link( $category[0]->term_id ) . '" title="' . sprintf( __( "View all posts in %s", 'greatwall' ), $category[0]->name ) . '" ' . '>' . $category[0]->name.'</a> ';
    }    
}
endif;
...
if ( ! function_exists( 'greatwall_custom_toolbar_link' ) ) :

function greatwall_custom_toolbar_link($wp_admin_bar) {
    $args = array(
        'id' => 'happythemes',
        'title' => 'Upgrade to Pro Theme', 
        'href' => 'https://www.happythemes.com/wordpress-themes/greatwall/', 
        'meta' => array(
            'class' => 'happythemes', 
            'title' => '',
            'target'=> '_blank',
...
add_action('admin_bar_menu', 'greatwall_custom_toolbar_link', 999);
