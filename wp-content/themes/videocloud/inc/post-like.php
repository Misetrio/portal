...
function sl_enqueue_scripts() {
	...
	wp_localize_script( 'simple-likes-public-js', 'simpleLikes', array(
		...
		'like' => __( 'Like', 'videocloud' ),
		'unlike' => __( 'Unlike', 'videocloud' )
	) ); 
}
...
function process_simple_like() {
	...
	if ( !wp_verify_nonce( $nonce, 'simple-likes-nonce' ) ) {
		exit( __( 'Not permitted', 'videocloud' ) );
	}
...
function get_simple_likes_button( $post_id, $is_comment = NULL ) {
	...
	if ( already_liked( $post_id, $is_comment ) ) {
		...
		$title = __( 'Unlike', 'videocloud' );
		$icon = $icon_full;
	} else {
		$class = '';
		$title = __( 'Like', 'videocloud' );
		$icon = $icon_empty;
	}
...
function get_like_count( $like_count ) {
	...
	$count = '<span class="sl-count">' . $number . ' <em>' . esc_html('Likes', 'videocloud') . '</em></span>';
	return $count;
} // get_like_count()
...
function show_user_likes( $user ) { ?>        
	<table class="form-table">
		<tr>
			<th><label for="user_likes"><?php _e( 'You Like:', 'videocloud' ); ?></label></th>
			<td>
			<?php
			...
			<?php else : ?>
			<p><?php _e( 'You do not like anything yet.', 'videocloud' ); ?></p>
			<?php 
			endif; 
			wp_reset_postdata(); 
			?>
