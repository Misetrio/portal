 ...
 * @since      1.0.0
 */
class videocloud_Posts_Block_Widget extends WP_Widget {
	...
	function __construct() {
		...
		$widget_options = array(
			'classname'   => 'widget-videocloud-home-posts-block',
			'description' => __( 'Display post blocks on homepage. Only use for the "Homepage Content" widget area.', 'videocloud' )
		);

		// Create the widget.
		parent::__construct(
			'happythemes-home-posts-block',          // $this->id_base
			__( '&raquo; Home Posts Block', 'videocloud' ), // $this->name
	...
	function widget( $args, $instance ) {
		...
		echo $before_widget;
			...
			$query = apply_filters( 'videocloud_home_one_column_args', $args );
	...
	?>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'cat' ); ?>"><?php _e( 'Choose category', 'videocloud' ); ?></label>
			<select class="widefat" id="<?php echo $this->get_field_id( 'cat' ); ?>" name="<?php echo $this->get_field_name( 'cat' ); ?>" style="width:100%;">
				<?php $categories = get_terms( 'category' ); ?>
				<option value="0"><?php _e( 'All categories &hellip;', 'videocloud' ); ?></option>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'limit' ); ?>">
				<?php _e( 'Number of posts to show', 'videocloud' ); ?>
			</label>
