 ...
 * @since      1.0.0
 */
class subscribe_Popular_Widget extends WP_Widget {
	...
	function __construct() {
		...
		$widget_options = array(
			'classname'   => 'widget-subscribe-popular widget_posts_thumbnail',
			'description' => __( 'Display popular posts by comments with thumbnails.', 'subscribe' )
		);

		// Create the widget.
		parent::__construct(
			'subscribe-popular',                                   // $this->id_base
			__( '&raquo; Popular Posts', 'subscribe' ), // $this->name
	...
	function widget( $args, $instance ) {
		...
		if ( false === ( $popular = get_transient( 'subscribe_popular_widget_' . $this->id ) ) ) {
			...
			// Store the transient.
			set_transient( 'subscribe_popular_widget_' . $this->id, $popular );
	...
	function flush_widget_transient() {
		delete_transient( 'subscribe_popular_widget_' . $this->id );
	}
	...
	function form( $instance ) {
		...
		// Default value.
		$defaults = array(
			'title'     => esc_html__( 'Popular Posts', 'subscribe' ),
			'limit'     => 5,
			'show_views' => true
		);
	...
	?>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'limit' ); ?>">
				<?php _e( 'Number of posts to show', 'subscribe' ); ?>
			</label>
		...
		<p>
			...
			<label for="<?php echo $this->get_field_id( 'show_views' ); ?>">
				<?php _e( 'Display post date?', 'subscribe' ); ?>
			</label>
		</p>
