...
class GreatWall_Pro_Portfolio extends WP_Widget {

	public function __construct() {
		$widget_ops = array('classname' => 'greawall_pro_portfolio_widget', 'description' => __( 'Display your projects in a grid.', 'greatwall') );
       parent::__construct(false, $name = __('Home - Portfolio', 'greatwall'), $widget_ops);
		$this->alt_option_name = 'greawall_pro_portfolio_widget';
  }
  ...
  public function widget( $args, $instance ) {
    ...
    $options = array(
     ...
     'include'       => $includes,
     'filter'        => $show_filter,
     'show_all_text' => ! empty( $show_all_text ) ? $show_all_text : __('Show all', 'greatwall')
    );
 ...
 function form($instance) {
    ...
    $show_all_text = isset( $instance['show_all_text'] )  ? esc_html($instance['show_all_text']) : __('Show all', 'greatwall');

	?>

   <p><label for="<?php echo $this->get_field_id('title'); ?>"><?php _e('Title:', 'greatwall'); ?></label>
   <input class="widefat" id="<?php echo $this->get_field_id('title'); ?>" name="<?php echo $this->get_field_name('title'); ?>" type="text" value="<?php echo esc_attr($title); ?>" /></p>
   <p><label for="<?php echo $this->get_field_id( 'number' ); ?>"><?php _e( 'Number of projects to show (-1 shows all of them):', 'greatwall' ); ?></label>
   <input id="<?php echo $this->get_field_id( 'number' ); ?>" name="<?php echo $this->get_field_name( 'number' ); ?>" type="text" value="<?php echo $number; ?>" size="3" /></p>
   <p><label for="<?php echo $this->get_field_id('includes'); ?>"><?php _e('Enter the slugs (comma separated) for your categories or leave empty to show all projects.', 'greatwall'); ?></label>
   ...
   <label for="<?php echo $this->get_field_id( 'show_filter' ); ?>"><?php _e( 'Show navigation filter? (Category slugs must be specified).', 'greatwall' ); ?></label></p>
	 <p><label for="<?php echo $this->get_field_id('show_all_text'); ?>"><?php _e('"Show all" text:', 'greatwall'); ?></label>
   ...
   <label for="<?php echo $this->get_field_id( 'show_project_title' ); ?>"><?php _e( 'Show project title?', 'greatwall' ); ?></label></p>
