...
class subscribe_Portfolio extends WP_Widget {

	public function __construct() {
		$widget_ops = array('classname' => 'subscribe_portfolio_widget', 'description' => __( 'Display your projects in a grid.', 'subscribe') );
       parent::__construct(false, $name = __('Home - Portfolio', 'subscribe'), $widget_ops);
		$this->alt_option_name = 'subscribe_portfolio_widget';
  }
  ...
  public function widget( $args, $instance ) {
    ...
    $options = array(
     'posts'         => $number,
     'post_type'     => 'projects',
     'include_cat'       => $includes,
     'filter'        => $show_filter,
     'show_all_text' => ! empty( $show_all_text ) ? $show_all_text : __('Show all', 'subscribe')
    );

    $output = ''; //Start output
    $output .= '<div class="project-wrap portfolio-section">';
    if ($options['include_cat'] && $options['filter'] == 1) {
       $included_terms = explode( ',', $options['include_cat'] );
       $included_ids = array();

       foreach( $included_terms as $term ) {
           $term_obj = get_term_by( 'slug', $term, 'project_category');
       ...
       $terms = get_terms( 'project_category', array( 'include_cat' => $id_string ) );
      ...
      $the_query = new WP_Query( array (
       ...
       'tax_query' => array(
          array(
            ...
            'field' => 'slug',
            'terms' => explode(',', $options['include_cat']),
          )
        )
      ) );
       ...
       if ( has_post_thumbnail() ) {
           ...
           if ( $project_url ) :
               ...
               $output .= '<div class="project-inner">';
							 $output .= '<a class="thumbnail-link" href="' . esc_url($project_url) . '">';
							 $output .= get_the_post_thumbnail($post->ID,'project_thumb');
							 $output .= '</a>';
               $output .= '<a class="project-title" href="' . esc_url($project_url) . '">';
               $output .= ($show_project_title == 1) ? get_the_title() : '';
               $output .= '</a>';               
               $output .= '</div>';
							 $output .= '</div>';
           else :
               $output .= '<div class="project-item item isotope-item ' . $termsString . '">';
               $output .= '<div class="project-inner">';
               $output .= '<a class="thumbnail-link" href="' . get_the_permalink() . '">';
               $output .= get_the_post_thumbnail($post->ID,'project_thumb');
               $output .= '</a>';
               $output .= '<h3 class="project-title"><a href="' . get_the_permalink() . '">';
               $output .= ($show_project_title == 1) ? get_the_title() : '';
               $output .= '</a></h3>';               
               $output .= '</div>';
               $output .= '</div>';               
 ...
 function form($instance) {
    ...
    $show_all_text = isset( $instance['show_all_text'] )  ? esc_html($instance['show_all_text']) : __('Show all', 'subscribe');

	?>

   <p><label for="<?php echo $this->get_field_id('title'); ?>"><?php _e('Title:', 'subscribe'); ?></label>
   <input class="widefat" id="<?php echo $this->get_field_id('title'); ?>" name="<?php echo $this->get_field_name('title'); ?>" type="text" value="<?php echo esc_attr($title); ?>" /></p>
   <p><label for="<?php echo $this->get_field_id( 'number' ); ?>"><?php _e( 'Number of projects to show (-1 shows all of them):', 'subscribe' ); ?></label>
   <input id="<?php echo $this->get_field_id( 'number' ); ?>" name="<?php echo $this->get_field_name( 'number' ); ?>" type="text" value="<?php echo $number; ?>" size="3" /></p>
   <p><label for="<?php echo $this->get_field_id('includes'); ?>"><?php _e('Enter the slugs (comma separated) for your categories or leave empty to show all projects.', 'subscribe'); ?></label>
   ...
   <label for="<?php echo $this->get_field_id( 'show_filter' ); ?>"><?php _e( 'Show navigation filter? (Category slugs must be specified).', 'subscribe' ); ?></label></p>
	 <p><label for="<?php echo $this->get_field_id('show_all_text'); ?>"><?php _e('"Show all" text:', 'subscribe'); ?></label>
   ...
   <label for="<?php echo $this->get_field_id( 'show_project_title' ); ?>"><?php _e( 'Show project title?', 'subscribe' ); ?></label></p>
