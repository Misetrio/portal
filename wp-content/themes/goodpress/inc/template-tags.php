...
function goodpress_get_post_views($postID){
    ...
    return '<strong class="view-count">' . number_format($count) . '</strong> ' . __('Views', 'goodpress');
}
...
function goodpress_custom_meta_box_markup($object)
{
    ...
    ?>
        <div>
            <label for="is_featured"><?php echo __('Featured this post on homepage', 'goodpress'); ?> </label>
            <?php
...
function goodpress_first_category() {
    ...
    if ($category) {
      echo '<a href="' . get_category_link( $category[0]->term_id ) . '" title="' . sprintf( __( "View all posts in %s", 'goodpress' ), $category[0]->name ) . '" ' . '>' . $category[0]->name.'</a> ';
    }    
}
endif;
    ...
    if ( isset($sidebars_widgets['homepage']) ) {
        if ( is_home() && is_array($sidebars_widgets['homepage']) ) {
            foreach($sidebars_widgets['homepage'] as $i => $widget) {
                ...
                if( (strpos($widget, 'happythemes-ad') !== false) ) {
                    unset($sidebars_widgets['homepage'][$i]);
                }                                      
            }
        }
...
if ( ! function_exists( 'goodpress_custom_toolbar_link' ) ) :

function goodpress_custom_toolbar_link($wp_admin_bar) {
    $args = array(
        'id' => 'happythemes',
        'title' => 'Upgrade to Pro Theme', 
        'href' => 'https://www.happythemes.com/wordpress-themes/goodpress/', 
        'meta' => array(
            'class' => 'happythemes', 
            'title' => '',
            'target'=> '_blank',
...
add_action('admin_bar_menu', 'goodpress_custom_toolbar_link', 999);
